---
name: peer-review
description: "Quick gut check from a different LLM via CLI (command-code or Claude Code `claude -p`). Bounce ideas, catch blind spots, find simpler alternatives. Lighter than @grill, faster than @review. Invoke explicitly; not a substitute for @review or @verify."
allowed-tools:
  - Bash
  - Read
  - Grep
...
Bounce an idea off a different LLM. Not a code review (`@review`), not adversarial (`@grill`).
...
- **Not** after `@test` for the PR gate — that's `@verify` (and checklist review is `@review`)
